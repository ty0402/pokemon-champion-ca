(function () {
  ...
  /** 将当前选中的对手模板排到筛选列表最前，便于收起列表时仍置顶可见 */
  function orderMetaWithSelectedFirst(filteredIndexes) {
    const sel = state.selectedMeta;
    if (!filteredIndexes.includes(sel)) return filteredIndexes;
    return [sel, ...filteredIndexes.filter((i) => i !== sel)];
  }
  ...
  function renderMeta() {
    ...
    const orderedIndexes = orderMetaWithSelectedFirst(filteredIndexes);
    const visibleIndexes = state.metaExpanded ? orderedIndexes : orderedIndexes.slice(0, 5);
